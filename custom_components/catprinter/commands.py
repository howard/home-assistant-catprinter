"""Cat printer wire protocol: command framing and image-row encoding.

This module is vendored verbatim (aside from this header) from the excellent
rbaron/catprinter project's ``catprinter/cmds.py``:

    https://github.com/rbaron/catprinter  (MIT License, Copyright 2021 rbaron)

The protocol was reverse-engineered there; see that repository for details. It
is pure Python with no third-party dependencies, so we keep it as the single
source of truth for the byte-level protocol and build the Home Assistant
integration on top of it.

The public entry point is :func:`cmds_print_img`, which takes an iterable of
image rows (each an iterable of truthy/falsy values where truthy == black pixel
to burn) and an ``energy`` value, and returns the full ``bytearray`` to send to
the printer over BLE.
"""
